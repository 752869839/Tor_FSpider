...
def phone_extract(text):
    phone_exp = re.compile('(?<!\\d)((?:13[0-9]|14[579]|15[0-3,5-9]|16[6]|17[0135678]|18[0-9]|19[89])\\d{8})(?!\\d)',
                           re.M | re.S | re.I)
...
"""QQ"""
REGEX_QQ = re.compile(r'[1-9]\d{6,10}', re.M | re.S | re.I)
def qq_extract(content):
    qq = re.findall(REGEX_QQ, content)
    return list(set(qq))

"""Wechart"""
REGEX_WX = re.compile(r'^微信.*?((?:1[3-9]\d{9}|[a-zA-Z\d._-]*\@[a-zA-Z\d.-]{1,10}\.[a-zA-Z\d]{1,20}|[\w,-]{1,20}|[1-9]\d{4,10}))$', re.M | re.S | re.I)
def wechart_extract(content):
    wechart = []
    pay = re.match(REGEX_WX, content)
    try:
        chart = pay.group(1)
...
"""Identity Card"""
card = re.compile(r'(?<!\d)(?:(?:([1-9]\d{5}(?:18|19|(?:[23]\d))\d{2}(?:(?:0[1-9])|(?:10|11|12))(?:(?:[0-2][1-9])|10|20|30|31)\d{3}[0-9Xx])(?!\d))|([1-9]\d{5}\d{2}(?:(?:0[1-9])|(?:10|11|12))(?:(?:[0-2][1-9])|10|20|30|31)\d{3}))(?!\d)', re.M | re.S | re.I)
def card_extract(content):
    identity_card = re.findall(card, content)
    identity_card = list(map(lambda x:''.join(x) ,identity_card))
    return identity_card

"""TG群"""
def tg_extract(content):
    tg_url_exp = re.compile(u'(?:http://|https://)(?:t\\.me|telegram\\.me)[a-z0-9-~\\/]*', re.M | re.S | re.I)
    tmp = tg_url_exp.findall(content)
    return list(set(tmp))


"""Alipay"""
REGEX_PAY = re.compile(r'^支付宝.*?((?:1[3-9]\d{9}|[a-zA-Z\d._-]*\@[a-zA-Z\d.-]{1,10}\.[a-zA-Z\d]{1,20}))$', re.M | re.S | re.I)
