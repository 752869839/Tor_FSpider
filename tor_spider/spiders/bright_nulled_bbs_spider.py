...
class DarkSpider(scrapy.Spider):
    name = 'bright_nulled_bbs_spider'
    # allowed_domains = ['nulled.to/']
    ...
    def parse(self, response):
        logger.info('开始采集!!!')
        item = HtmlItem()
        list_urls = response.xpath('//div[@id="category_4"]//h4[@class="forum_name"]/strong/a/@href').extract()
    ...
    def parse_sencond(self, response):
        ...
        list_urls = response.xpath('//a[@class="topic_title highlight_unread"]/@href').extract()
        for list_url in list_urls:
            list_url = response.urljoin(list_url)
