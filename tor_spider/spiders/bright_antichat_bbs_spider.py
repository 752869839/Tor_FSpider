...
class DarkSpider(scrapy.Spider):
    name = 'bright_antichat_bbs_spider'
    ...
    def parse(self, response):
        logger.info('开始采集!!!')
        item = HtmlItem()
        list_urls = response.xpath('//h3[@class="nodeTitle"]/a/@href|//h4[@class="nodeTitle"]/a/@href').extract()
    ...
    def parse_sencond(self, response):
        ...
        list_urls = response.xpath('//a[@class="PreviewTooltip"]/@href').extract()
        for list_url in list_urls:
            list_url = response.urljoin(list_url)
