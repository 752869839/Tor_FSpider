...
class DarkSpider(scrapy.Spider):
    name = 'bright_cybernews_bbs_spider'
    ...
    def parse(self, response):
        logger.info('开始采集!!!')
        item = HtmlItem()
        list_urls = response.xpath('//h3[@class="jeg_post_title"]/a/@href').extract()
        ...
        try:
            next_page = response.xpath('//a[@class="page_nav next"]/@href').extract()[0]
