...
class DarkSpider(RedisSpider):
    ...
    # start_urls = ['https://www.nytimes3xbfgragh.onion/']
    ...
    def parse(self, response):
        item = HtmlItem()
        urls = response.xpath('//a[@lang="en-US"]/@href|//a[@lang="zh-hans"]/@href').extract()  # 这是下面的匹配
        for url in urls:
    ...
    def parse_second(self,response):
        ...
        try:
            ...
            pass
        urls = response.xpath('//ul[@class="css-1iruc8t"]/li/a/@href|//li[@class="mainSection  drop"]/a/@href|//li[@class="mainSection "]/a/@href').extract()
        for url in urls:
            url = response.urljoin(url)
            logger.info('请求链接')
            logger.info(url)
    ...
    def parse_third(self,response):
        ...
        details_urls = response.xpath('//li[@class="css-ye6x8s"]//a/@href|//h3[@class="regularSummaryHeadline"]/a/@href').extract()
